...
The exact motion planning algorithms do not scale to more complex problems as they make certain limiting assumptions about the geometry of the robot and the obstacles. In contrast, the modern approaches use approximations of the general motion planning problem.
Since the exact methods are no more used in modern days, the computational complexity analysis for the general motion planning problem is of less relevance now and the more recent research literature focusses on the analysis of the specific algorithms such as sampling or search-based algorithms and is based on their respective approximation schemes.
...
Note that the running time does not have to be independent of~$n$, but but its upper bound has to be bounded independently of~$n$. In Big O notation, a constant-time algorithm has an asymptotic complexity denoted as~$O(1)$.
...
\begin{definition}
\label{ctmp:def}
A CTMP algorithm can plan for the next action under a fixed time budget~$\Tbound$ along a path that is guaranteed to have no cycles, where~$\Tbound$ has a (small) constant-time lower bound.
...
The time budget~$\Tbound$ is a \emph{user-specified} input to the algorithm. It should be greater than a minimum required time~$T_\textrm{min}$ for the CTMP algorithm. However,~$T_\textrm{min}$ is upper bounded by a small value that is independent of the size of the input to the algorithm (hence the term constant-time).

Additionally, it is assured that the complete path, if reconstructed does not contain cycles.
...
\begin{property}
\label{ctmp:prop1}
A CTMP algorithm requires~$O(1)$ time to plan the next action along a path from a start to goal
\end{property}
...
\begin{proof}
We can prove completeness by showing that (1) if~$g$ is reachable from~$s$, it is \emph{covered} by~$s$ in Alg. 3. (2) if~$g$ is covered by~$s$, Alg. 4 is guaranteed to return a path.
...
As the algorithm finds full path in~$O(1)$ time~(which is a stronger property), it can also find the next action along the full path in~$O(1)$ time. Hence we have the following corolary:
