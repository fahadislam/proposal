...
\newpage
\begin{abstract}
In manufacturing and warehouse scenarios, robots often operate in semi-structured environments. In these environments, the robots perform recurring manipulation tasks with no or little changes in the their surroundings.
...
In this thesis we have developed constant-time motion planning algorithms for four semi-structured domains; (1) Manipulation for repetitive tasks in static environments (2) Manipulation for repetitive tasks in semi-static environments (3) Manipulation for the task of picking up moving objects (of known models) off a conveyor belt (4) Manipulation for the task of protecting a robot with a shield from incoming attack.
Domain (1) assumes that the environment occupancy remains purely static during robot's operation.
Domain (2) relaxes the purely-static assumption by allowing a fixed number of movable obstacles that may get displaced during operation.
Domains (3) and (4) have dynamic environments. In these domains, the robot typically perceives a rough pose estimate viewing from a distance, but it must start moving early on (relying on that rough estimate) and then adjust its motion in real time as it gets improved estimates to be able to reach the object in time.

Our key insight is that in these semi-structured domains, the space in which the robot operates is a very small subset of its configuration space which allows us to preprocess it exhaustively. The preprocessing step generates a representative set of paths that can be used by search at query time in a way that assures small constant-time planning.
%
For domain (1), we evaluate our algorithm for a mail sorting task in simulation on PR2 and also test the algorithm on a real truck-unloading robot. For domain (2), we perform simulation and real-robot experiments on Franka arm for a pick and place task in a kitchen environment. For domain (3) we perform real-robot experiments on PR2 working at a conveyor belt. Finally, for domain (4) we perform simulation and real-robot experiments on PR2 shielding itself from incoming attack.
...
\label{def:complete}
    An algorithm \algname{ALG} is said to be CTMP-complete if it covers all the reachable goals $g \in G$ for \Sstart.
\end{definition}
...
%\begin{lemma}
%\label{lemma:o1}
%	For the case when ~$|\pi_g| \leq c \cdot |\pi_i|$, where~$c \geq 1$ is a constant integer, each action requires~$O(1)$ time delibration
%\end{lemma}
...
The average shielding factor which we obtain by averaging over all the cells on the dome's front side is 0.5. Note that this analysis doesn't take the time dimension into account. If we assume a minimum time of flight of the attack, we can additionally check if the precomputed motion plans can be executed within that time threshold and then discount the results if that check fails.
